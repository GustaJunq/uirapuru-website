...
export function UirapuruApp({ conversationId }: { conversationId?: string }) {
  ...
  async function pollAgent(runId: string, signal: AbortSignal) {
    while (!signal.aborted) {
      ...
      if (run.status === "FAILED") throw new ApiError(run.error || "O Uirapuru não conseguiu concluir a tarefa.")
    ...
    <main className="relative flex min-h-svh flex-col overflow-hidden bg-background text-foreground">
      {routeChat ? (
        <header className="chat-header chat-header-enter relative flex h-16 shrink-0 items-center justify-center px-4">
          ...
          <img src={logo} alt="Uirapuru" className="h-auto w-32 object-contain" />
        </header>
      ) : (
        <header className="relative flex w-full items-center justify-between px-4 py-4 md:px-8 md:py-5">
          ...
          </div><p className="text-sm font-semibold tracking-tight">Uirapuru Chat</p>
      ...
      {routeChat ? (
        <section className="chat-view-enter mx-auto flex w-full max-w-3xl flex-1 flex-col overflow-hidden px-5" aria-label="Conversa">
          <div className="chat-scroll flex flex-1 flex-col gap-6 overflow-y-auto pb-8 pt-8 md:px-1" aria-live="polite">
            ...
            {!loadingConversation && error && messages.length === 0 && <div className="m-auto text-center"><p className="text-sm text-muted-foreground">{error}</p><Button variant="outline" className="mt-4" onClick={() => loadHistory()}>{'Tentar novamente'}</Button></div>}
            {messages.map((message, index) => (
              <MessageRow key={index} message={message} />
            ))}
            <div ref={bottomRef} />
          </div>
          <Composer prompt={prompt} setPrompt={setPrompt} pending={pending} submit={submit} onKeyDown={handleKeyDown} stop={() => abortRef.current?.abort()} compact />
        ...
        <section className="flex flex-1 flex-col items-center px-4">
          <div className="hero-enter flex w-full flex-1 flex-col items-center justify-center gap-9 pb-24">
            <img src={logo} alt="Uirapuru" className="brand-logo h-auto w-52 object-contain" />
            <Composer prompt={prompt} setPrompt={setPrompt} pending={pending} submit={submit} onKeyDown={handleKeyDown} stop={() => abortRef.current?.abort()} />
...
function MessageRow({ message, mode }: { message: ChatMessage; mode: "UIRAPURU" }) {
...
function Composer({ prompt, setPrompt, pending, submit, onKeyDown, stop, compact = false }: { prompt: string; setPrompt: (value: string) => void; pending: boolean; submit: (event: FormEvent) => void; onKeyDown: (event: KeyboardEvent<HTMLInputElement>) => void; stop: () => void; compact?: boolean }) {
  return <form onSubmit={submit} className={`${compact ? "composer composer-dock-enter mb-3 flex w-full items-center" : "composer flex w-full max-w-[700px] items-center"}`}>
    ...
    <input id="message" value={prompt} onChange={(event) => setPrompt(event.target.value)} onKeyDown={onKeyDown} disabled={pending} autoComplete="off" placeholder="Manda aí — Uirapuru responde em português." className="mx-3 w-full bg-transparent text-sm placeholder:text-muted-foreground focus:outline-none" />
...
function Legal() { return <p className="pb-2 text-center text-[9px] leading-relaxed text-muted-foreground sm:text-[10px]">Ao trabalhar com Uirapuru você aceita os termos.</p> }
